...
class MatchingParens extends RecursiveDescent {
   ...
   /** Fill me in */
   public void S() {
	   if (peek() == sym.lparen) { // predicts S -> lparen S rparen
	         expect(sym.lparen);
	         S();
	         expect(sym.rparen);
	      }
	   else if (peek() == sym.x) { // predicts B -> d
	         expect(sym.x);
	      }
	   else oops("expected lparen or x");
   }
}
...
class Problem1a extends RecursiveDescent {
   ...
   /** Fill me in */
   public void S() {
	   if (peek() == sym.a || peek() == sym.b || peek() == sym.c ) { // predicts S -> A B c
	         A();
	         B();
	         expect(sym.c);
	      }
	   else oops("expected a,b or c");
   }
   
   public void A() {
	      if (peek() == sym.a) {      // predicts A -> a
	         expect(sym.a);
	      }
	      else if (peek() == sym.b || peek() == sym.c ) { // predicts A ->lambda
	      }
	      else oops("expected a,b or c");
	   }
   ...
   public void B() {
	      ...
	      else if (peek() == sym.c) { // predicts B -> lamda
	      }
	      else oops("expected b or c");
	   }
}
